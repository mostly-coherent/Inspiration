...
def detect_counter_intuitive(
    ...
    clusters = cluster_library_items(client, threshold)
    ...
    for idx, cluster in enumerate(strong_clusters[:max_suggestions]):
        ...
        # Generate counter-perspective
        print(f"   Generating counter-perspective for: {cluster_title}...", file=sys.stderr)
        result = generate_counter_perspective(cluster_title, cluster)
    ...
    print(f"   Generated {len(suggestions)} counter-perspectives", file=sys.stderr)
