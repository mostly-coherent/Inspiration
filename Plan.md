...
| Idea | Scope | Notes |
|------|-------|-------|
| Multi-source artifacts | Code comments + TODOs + workspace .md files only | NOT git/PR content (increasingly AI-generated) |
| Socratic mode | Full Setup only (requires Vector DB + Library) | Built on Theme Explorer data, NOT user KG (which was useless) |
...
| Change | Description |
|--------|-------------|
| **Removed user KG from scoreboard** | "Your Knowledge Graph" section removed entirely. KG indexing UI removed. Pages (/entities, /graph) still accessible via direct URL. |
| **Folded Lenny KG into Archive** | Expert KG stats displayed within "Wisdom from Lenny's Podcasts" section (was already there). |
| **Memory source breakdown** | Source breakdown shows "Docs: N files" alongside Cursor and Claude Code counts. |
| **Counter-Intuitive → Reflect tab** | Counter-Intuitive tab replaced with "Reflect" tab. Theme Explorer now: Patterns, Unexplored, Reflect. |
...
| MS-2 | **Ingestion Adapter** | Feed workspace docs into existing Vector DB pipeline (`source="workspace_docs"`) | ✅ Done |
...
- `engine/scripts/sync_messages.py` — `sync_workspace_docs()` function added
- `src/app/api/brain-stats/sources/route.ts` — Enhanced with workspace docs count
- `src/components/ScoreboardHeader.tsx` — Source breakdown includes docs

### Phase 2: Socratic Mode (Reflect Tab)

| ID | Feature | Description | Status |
|----|---------|-------------|--------|
| SOC-1 | **Data Aggregation** | Aggregates patterns, unexplored, counter-intuitive, library stats, expert matches, temporal shifts | ✅ Done |
| SOC-2 | **Question Generation** | LLM prompt generates 8-12 probing questions with evidence citations | ✅ Done |
| SOC-3 | **API Route** | `GET/POST /api/themes/socratic` with 24h cache | ✅ Done |
| SOC-4 | **Reflect Tab UI** | Question cards with category badges, difficulty indicators, Sit With This / Not Relevant actions | ✅ Done |
| SOC-5 | **Cross-Source Expert Matching** | Top patterns matched against Lenny's archive for expert divergence questions | ✅ Done |
| SOC-6 | **Counter-Intuitive Merge** | Counter-Intuitive tab absorbed into Reflect tab | ✅ Done |
...
- `src/components/ThemeExplorerTabs.tsx` — Tab config updated

**Question Categories:**
1. **Pattern** — Why does X dominate while Y barely exists?
2. **Gap** — You discussed X in 20 conversations but never formalized insights. Why?
3. **Tension** — Your Library has both X and anti-X. How do you reconcile?
4. **Temporal** — X disappeared 3 months ago. What changed?
5. **Expert** — Experts treat X as foundational. You've never mentioned it.
6. **Alignment** — Your docs prioritize X but your chats focus on Y.
...
| Metric | Target |
|--------|--------|
| Workspace docs in Memory | >0 after first sync |
| Socratic questions per generation | 8-12 |
| "Sit With This" engagement rate | >30% |
| Questions citing specific evidence | 100% |
| At least 2 "uncomfortable" per generation | Yes |

---

**Last Updated:** 2026-02-06 (v6 Socratic Mode + Multi-Source Memory implementation complete)
